          ...
          <tr>
            <td>
              <div class="product-admin-row">
                <div class="product-thumb-admin">${p.emoji || '🧱'}</div>
                <div>
...
window.saveProduct = function () {
  ...
  if (editingProductId) {
    ...
    if (idx !== -1) {
      products[idx] = { ...products[idx], name, category, price, unit, stock, description, dimensions, weight, emoji, outOfStockReason };
    }
    showToast('Product updated successfully!', 'success');
  } else {
    const newId = products.length > 0 ? Math.max(...products.map(p => p.id)) + 1 : 1;
    products.push({ id: newId, name, category, price, unit, stock, description, dimensions, weight, emoji, outOfStockReason, image: null, featured: false });
...
window.updateOrderStatus = function (orderId, newStatus) {
  ...
  if (newStatus === 'Confirmed') {
    ...
    const itemLines = order.items.map(i => `  • ${i.name} × ${i.qty} — ₹${i.total.toLocaleString('en-IN')}`).join('\n');
    const deliveryLine = order.delivery === 0 ? 'Free 🎉' : `₹${order.delivery.toLocaleString('en-IN')}`;
    const address = [order.customer.address, order.customer.village, order.customer.district, order.customer.pin].filter(Boolean).join(', ');
    const msg = `🧱 *${s.storeName} — Order Confirmed!* ✅

Hello *${order.customer.name}*! Your order has been confirmed and is being processed.
...
_Thank you for choosing ${s.storeName}! 🙏_`;
    if (window.confirm(`Send WhatsApp confirmation to ${order.customer.name} (${order.customer.phone})?`)) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (page === 'orders.html') {
    ...
    if (_newCount > 0) {
      const _ordersLink = document.querySelector('a[href="orders.html"]');
      if (_ordersLink) {
